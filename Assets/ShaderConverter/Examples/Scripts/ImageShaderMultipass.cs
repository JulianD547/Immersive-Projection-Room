...
namespace ReformSim
{
    public class ImageShaderMultipass : ImageShaderInput
    {
        ...
        protected override void Start()
        {
            ...
            if (m_bufferA == null)
            {
                m_bufferA = new RenderTexture(Screen.width, Screen.height, 0, GraphicsFormat.R32G32B32A32_SFloat);
                m_bufferA.useMipMap = true;
                m_bufferA.Create();
            }
            m_bufferA.DiscardContents();

            if (m_bufferB == null)
            {
                m_bufferB = new RenderTexture(Screen.width, Screen.height, 0, GraphicsFormat.R32G32B32A32_SFloat);
                m_bufferB.useMipMap = true;
                m_bufferB.Create();
            }
            m_bufferB.DiscardContents();

            if (m_bufferC == null)
            {
                m_bufferC = new RenderTexture(Screen.width, Screen.height, 0, GraphicsFormat.R32G32B32A32_SFloat);
                m_bufferC.useMipMap = true;
                m_bufferC.Create();
            }
            m_bufferC.DiscardContents();

            if (m_bufferD == null)
            {
                m_bufferD = new RenderTexture(Screen.width, Screen.height, 0, GraphicsFormat.R32G32B32A32_SFloat);
                m_bufferD.useMipMap = true;
                m_bufferD.Create();
            }
            m_bufferD.DiscardContents();
        ...
        protected override void UpdateMateialProperties(Material mat)
        {
            base.UpdateMateialProperties(mat);

            base.UpdateMateialProperties(m_bufferAMaterial);
            if (m_bufferAMaterial != null && m_bufferA != null)
            {
                //m_bufferAMaterial.SetTexture("_MainTex", m_texture1);
                //m_bufferAMaterial.SetTexture("_SecondTex", m_bufferA);
                Graphics.Blit(null, m_tempBufferA, m_bufferAMaterial);
                Graphics.Blit(m_tempBufferA, m_bufferA);
            }

            base.UpdateMateialProperties(m_bufferBMaterial);
            if (m_bufferBMaterial != null && m_bufferB != null)
            {
                ...
                Graphics.Blit(null, m_tempBufferB, m_bufferBMaterial);
                Graphics.Blit(m_tempBufferB, m_bufferB);
            }

            base.UpdateMateialProperties(m_bufferCMaterial);
            if (m_bufferCMaterial != null && m_bufferC != null)
            {
                Graphics.Blit(null, m_tempBufferC, m_bufferCMaterial);
                Graphics.Blit(m_tempBufferC, m_bufferC);
            }

            base.UpdateMateialProperties(m_bufferDMaterial);
            if (m_bufferDMaterial != null && m_bufferD != null)
            {
                Graphics.Blit(null, m_tempBufferD, m_bufferDMaterial);
...
//#endif
        ...
        public void ShowRenderTextures(bool showRenderTexture)
        {
            if (showRenderTexture)
            {
                if (m_bufferA != null)
                {
                    GUI.DrawTexture(new Rect(0, 0, m_bufferA.width / 4.0f, m_bufferA.height / 4.0f), m_bufferA, ScaleMode.StretchToFill, false);
                }
                if (m_bufferB != null)
                {
                    GUI.DrawTexture(new Rect(0, m_bufferB.height / 4.0f + 1, m_bufferB.width / 4.0f, m_bufferB.height / 4.0f), m_bufferB, ScaleMode.StretchToFill, false);
                }
                if (m_bufferC != null)
                {
                    GUI.DrawTexture(new Rect(0, m_bufferC.height / 2.0f + 2, m_bufferC.width / 4.0f, m_bufferC.height / 4.0f), m_bufferC, ScaleMode.StretchToFill, false);
                }
                if (m_bufferD != null)
                {
                    GUI.DrawTexture(new Rect(0, m_bufferD.height * 3.0f / 4.0f + 2, m_bufferD.width / 4.0f, m_bufferD.height / 4.0f), m_bufferD, ScaleMode.StretchToFill, false);
                }
            }
        }
