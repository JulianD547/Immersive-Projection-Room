    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            void frag_main()
            {
                ...
                {
                    _439 = (frac(-_Time.y) * 0.100000001490116119384765625f) * sin(30.0f * _Time.y);
                }
                else
                {
                    _439 = 0.0f;
                ...
                float wobble = _439;
                float3 dir = normalize(float3(((_texCoord_ * _iResolution.xy) * 2.0f) - _iResolution.xy, _iResolution.y));
                ...
                if (t < 12.0f)
                {
                    ...
                    float _543 = refr(param_5, param_6, param_7, param_8, param_9, param_10, param_11);
                    float t2 = param_10;
                    nor2 = param_11;
                    color2 += (_543 * depth).xxx;
                ...
                for (int i = 0; i < 128; i++)
                {
                    ...
                    float _584 = _noise(param_12);
                    float nebula = _584;
                    float3 param_13 = org + (nor2 * 0.5f);
                    float _591 = map(param_13);
                    density = intensity - (_591 * nebula);
                    if (density > 0.0f)
                    {
                        tmp = density / 128.0f;
                        T *= (1.0f - (tmp * 100.0f));
                ...
                float _651 = _noise(param_14);
                color2 += (((1.0f - depth) * _651) * 0.100000001490116119384765625f).xxx;
